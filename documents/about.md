# About

I didn't start as a designer. I studied engineering, worked as a PM at a digital agency and an accounting software company, and somewhere along the way realized that the products I was managing would be better if I just designed them myself.

That shift happened over six years ago. Since then, I've been designing B2B products in maritime and HR — industries where the workflows are dense, the users are busy, and there's zero room for pretty-but-confusing interfaces.

My engineering background means I think in systems. My PM years mean I think in trade-offs, stakeholder language, and delivery timelines. Both of those make me faster — once I understand the context and the required outcomes, screens come together quickly. People who've worked with me call it fast fingers.

I also know enough code to be dangerous. I've built two apps from scratch — Meetz for room booking, and Optz Trkz for options tracking. I'm not writing production code in my day job, but it means I speak the same language as the engineers I work with.

When I'm not designing, I'm either on a bike or in the gym chasing the leanest possible version of Arnold Schwarzenegger. Otherwise I'm probably with my family.

# logo

The logo is made up of my initials, D and L. It's rotated 90 degrees clockwise to resemble my Chinese surname 呂. The line connecting the two initials reminds me to connect to the users to understand the problems we are solving for them.

# experience

1. Senior product designer
   1. BIPO Service: nov 2025 - current

1. Product designer
   1. OmniHR: jun 2022 - nov 2025

1. UXUI designer
   1. Treehouse Finance: nov 2021 - jun 2022

1. Project manager / product designer
   1. Claritecs: jun 2019 - nov 2021

1. Project manager
   1. Codigo: aug 2018 - jun 2019

1. E-commerce website designer
   1. Growthwell Group: jun 2017 - jul 2018

1. Project manager
   1. TNO Systems: may 2014 - sep 2016

# clients

1. whilemsen shipboker
1. super steel pte ltd
1. super bend pte ltd
1. qianyu sg
1. tetraexcellence pte ltd
1. ground-up initiative
1.
